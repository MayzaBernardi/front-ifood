        ...
        const buscarCardapio = async () => {
            try {
                const resposta = await api.get(
                    `/cardapios/restaurantes/${id_restaurante}`
                );
                ...
                setCardapio(
                    Array.isArray(listaPratos) ? listaPratos : []
                );
            } catch (error) {
                console.error(error);
            } finally {
    ...
    return (
        <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
            ...
            <main className="max-w-4xl mx-auto p-4 sm:p-8">
                <button
                    onClick={() => router.back()}
                    className="flex items-center gap-2 text-ifood font-bold mb-6 hover:underline"
                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {cardapio.length > 0 ? (
                            cardapio.map((item) => (
                                <div
                                    key={item.id}
                                    className="flex justify-between items-center p-4 border-gray-200 rounded-lg bg-white shadow-sm"
                                >
                                    <div className="flex gap-4 items-center">
                                    <img
                                        src={`http://localhost:3333/${item.arquivos_cardapios[0].caminho_arquivo}`}
                                        alt={item.nome_prato}
                            ...
                            <p className="text-gray-500">
                                Nenhum prato encontrado neste restaurante.
                            </p>
                        )}
                    </div>
                )}
